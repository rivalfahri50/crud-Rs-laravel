...
<div class="container mt-5">
    ...
    &nbsp; &nbsp;<a href="{{ route('pembayaran.create') }}" class="btn btn-warning">Add Data</a>
...
<table class="table table-striped">
    ...
    <tr>
        ...
        <td>
            {{$pembayaran->nama_dokter}}
        </td>
        <td>
            {{$pembayaran->obat}}
        </td>
        <td>
            {{$pembayaran->jumlah_obat}}
        </td>
        <td>
            {{$pembayaran->ruang}}
        </td>
        <td>
          ...
          <form id="delete-form-{{ $pembayaran->id }}" action="{{route('pembayaran.destroy', $pembayaran->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <a href="{{ route('pembayaran.edit',$pembayaran->id) }}" class="btn btn-primary">Edit</a>
            <button type="submit" class="btn btn-danger" onclick="showAlert(event, {{ $pembayaran->id }})">Delete</button>
        </form>
    </center>
        ...
        function showAlert(event, id) {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    swal("Data pembayaran tidak dihapus.");
                }
